# סטטוס — 2026-09-04

## 🟢 המערכת חיה ועובדת מקצה לקצה

הבוט ענה בפועל בוואטסאפ ב-16:13 UTC. המחזור המלא אומת בלוגים של השרת:

```
16:12:45  שיחת Gemini נפתחה (AFC enabled)
16:13:00  POST .../gemini-3.6-flash:generateContent  →  200 OK
16:13:02  POST /webhook                              →  200
16:13:03  אישור מסירה חזר, זוהה כסטטוס ונענה נכון
```

## מה מחובר

| רכיב | מצב |
|---|---|
| Gemini (`google.genai`, `gemini-3.6-flash`) | ✅ עובד |
| שרת Flask ב-Render | ✅ Live — https://personal-assistant-y754.onrender.com |
| Meta WhatsApp Cloud API — אימות webhook | ✅ עבר (200 על ה-handshake) |
| הרשמה לשדה `messages` | ✅ Subscribed |
| חיבור האפליקציה ל-WABA (`subscribed_apps`) | ✅ `success: true` |
| מספר הטלפון האישי כנמען מאומת | ✅ |
| Google Calendar (יומן העבודה) | ✅ קריאה וכתיבה אומתו בפועל |
| 29 בדיקות pytest | ✅ ירוקות |

**מזהים:** App ID `1692195671856956` · WABA ID `3170938719759730` · Phone Number ID `1301270403072151` · מספר בדיקה `+1 555 204-1960`

## ✅ טוקן קבוע הותקן (2026-09-05)

הטוקן הזמני (24 שעות) הוחלף בטוקן **System User ללא תפוגה**, ולכן הבוט לא ייפול מעצמו.

- System User: `whatsapp-bot` (ID `61594093921569`), הרשאת Admin, גישה מלאה לאפליקציה "עוזר אישי"
- הרשאות הטוקן: `whatsapp_business_messaging`, `whatsapp_business_management`
- תפוגה: **Never**

אומת בלוגים ב-20:13-20:14 UTC: שלושה מחזורי הודעה מלאים, כולל קריאת פונקציה אחת (`AFC remote call 1 is done`), ואפס שגיאות שליחה.

## יומן Google — חובר 2026-09-05

- Service Account: `calendar-bot@gen-lang-client-0890389089.iam.gserviceaccount.com`
- **היומן המחובר הוא יומן העבודה: `itai.samsung.isr@gmail.com`** — לא החשבון הפרטי. האירועים האמיתיים (ימי חפיפה, ביקורי סניפים) יושבים שם.
- אומת מקצה לקצה: יצירת אירוע → קריאה חזרה → מחיקה, וגם שאילתה אמיתית דרך העוזר בוואטסאפ שהחזירה 11 אירועים מסודרים לפי ימים.
- נוסף מנגנון **רענון סשן יומי**: התאריך הנוכחי מוזרק ל-system instruction, וסשן שנוצר אתמול נבנה מחדש היום — אחרת העוזר היה קובע אירועים ביום הלא נכון.

**היומן הפרטי (`itaiel13@gmail.com`) משותף גם הוא אך אינו בשימוש כרגע.** אם נרצה שהעוזר יימנע מהתנגשות עם אירועים אישיים, צריך להרחיב לקריאה משני יומנים.

## מכסת Gemini — הוחלף מודל 2026-09-05

נתקלנו בקיר אמיתי בייצור: **`gemini-3.6-flash` בתוכנית החינמית מוגבל ל-20 בקשות ליום**
(`GenerateRequestsPerDayPerProjectPerModel-FreeTier`). כל הודעת וואטסאפ צורכת 2-3 בקשות
בגלל לולאת קריאות הפונקציות — כלומר **7-8 הודעות ביום** בפועל.

**הפתרון הזמני:** מעבר ל-`gemini-flash-lite-latest` (משתנה סביבה `GEMINI_MODEL_NAME`, בלי שינוי קוד).
נבדקו שלוש גרסאות lite מול היומן האמיתי — כולן קראו נכון לכלי, אבל `gemini-3.1-flash-lite`
**המציא שעת סיום לאירוע** שהכלי מעולם לא החזיר, ולכן נפסל. הנבחר דייק בעובדות.

**עלות המעבר לתשלום, אם יוחלט (נמדד, לא מוערך):** ~4,640 טוקני קלט + 400 פלט להודעה.
לפי מחירון ההשקה ($0.75/$3.75 למיליון): 10 הודעות/יום ≈ **$1.5/חודש**, 30/יום ≈ **$4.5**,
60/יום ≈ **$9**. המחירים ממקורות משניים — עמוד המחירון של Google חסום מסביבת הפיתוח.

## מגבלות ידועות (לא באגים)

- **Render בטיר החינמי נרדם** אחרי ~15 דקות חוסר פעילות. ההודעה הראשונה אחרי שינה עלולה לקחת 30-60 שניות. מעבר לטיר בתשלום פותר.
- **זיכרון שיחה קצר-טווח מתאפס** בכל הפעלה מחדש של השרת (כולל אחרי שינה). עובדות בזיכרון ארוך-הטווח נשמרות.
- **מספר הבדיקה של Meta מוגבל ל-5 נמענים מאומתים.** למספר עסקי אמיתי צריך Business Verification (Step 3 בממשק).
- **רק הודעות טקסט נתמכות.** הקלטה קולית/תמונה מקבלות תשובה מנומסת שמסבירה זאת.
- `get_itai_targets` ו-`update_daily_schedule` הם עדיין placeholders — אין חיבור אמיתי ל-Google Sheets.

## הצעד הבא המתבקש

חיבור אמיתי ל-Google Sheets, כדי שהעוזר יקרא ויכתוב נתוני יעדים וביקורים אמיתיים במקום הנתונים הקבועים שיש עכשיו. דורש Service Account מ-Google Cloud + שיתוף הגיליון איתו.
